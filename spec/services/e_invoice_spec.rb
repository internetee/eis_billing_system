...
RSpec.describe 'EInvoice' do
  ...
  describe 'e_invoice format' do
    it 'Should convert request data of invoice to e_invoice format' do
      e_invoice_instance = EInvoiceGenerator.new(invoice_data).generate

      expect(e_invoice_instance.invoice.seller.name).to eq('Eesti Interneti SA')
      expect(e_invoice_instance.invoice.buyer.name).to eq('ACCREDITATION EIS')
      expect(e_invoice_instance.invoice.subtotal).to eq('0.833')
    end
  end
end
