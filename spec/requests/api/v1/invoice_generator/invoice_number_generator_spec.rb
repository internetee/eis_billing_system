...
RSpec.describe "Api::V1::InvoiceGenerator::InvoiceNumberGenerators", type: :request do
  ...
  describe "POST /create" do
    ...
    it "should generate number for invoice which will be one more than the previous number" do
      invoice.invoice_number = Api::V1::InvoiceGenerator::InvoiceNumberGeneratorController::INVOICE_NUMBER_MIN
      invoice.save
    ...
    it "if no any invoice should be return minimum range of invoice number" do
      ...
      expect(response).to have_http_status(:success)
      expect(JSON.parse(response.body)['invoice_number']).to eq(Api::V1::InvoiceGenerator::InvoiceNumberGeneratorController::INVOICE_NUMBER_MIN)
    end
  end
end
